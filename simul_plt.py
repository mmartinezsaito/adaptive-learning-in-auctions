...
#########################################################
###  Simulation of agents with discriminative models  ###
#########################################################
...
def play_avf(params):
    ...
    nb = max(D.ix[:, 'block_n'])
    emv = {m:[[] for i in range(nb)] for m in markets}
    opb = {m:[[] for i in range(nb)] for m in markets}
    rpe = {m:[[] for i in range(nb)] for m in markets}
    ecv = {m:[[] for i in range(nb)] for m in markets}
    sve = {m:[[] for i in range(nb)] for m in markets}
    ...
    cmr = {m:[[] for i in range(nb)] for m in markets}
    sib = {m:[[] for i in range(nb)] for m in markets}
    ...
    cw.writerow(["beh", "opb", "clh", "rpe", "cob", "coe", "cmr", "sve", "ecm", "ecv", "sib"])
    ...
    for m in markets:
        ...
        mn1 = map(sp.mean, fil1)
        mn2 = map(sp.mean, fil2)
        sem1 = map(sp.stats.sem, rpe[m])
        sem2 = map(sp.stats.sem, coe[m])
    ...
    plt.subplot(324)
    for m in range(3):
        bdB(bda[m], bdb[m], bdc, ax = axa[2][0])
...
def play_dr_nudger(params):
    ...
    coe = {m:[[] for i in range(nb)] for m in markets}
    cmr = {m:[[] for i in range(nb)] for m in markets}
    otb = {m:[[] for i in range(nb)] for m in markets}
    ...
    fid = open(fn, 'w') 
    cw = csv.writer(fid, delimiter = ',')
    cw.writerow(["beh", "opb", "clh", "rpe", "cob", "coe", "cmr", "otb", "sib"])
    ...
    plt.gca().set_ylim([minmn-1, maxmn+1])
    plt.subplot(321)
    plt.gca().set_color_cycle(['c', 'c', 'm', 'm', 'y', 'y'])
    ...
    for m in markets:
        ...
        mn1 = map(sp.mean, fil1)
        mn2 = map(sp.mean, fil2)
        sem1 = map(sp.stats.sem, sib[m])
        sem2 = map(sp.stats.sem, cob[m])
